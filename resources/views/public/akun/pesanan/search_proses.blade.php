@if(count($orders->where('transaction_status', 2)->where('payment_status', 2)) > 0)
<!-- Start Ada Pesanan -->
@foreach($orders->where('transaction_status', 2)->where('payment_status', 2) as $order_process)
<div class="pt-list-order">
    <div class="grid_layouts head-pt-list-order">
        <div class="bx-head-mns code-order">
            <h5 class="nocode main-sub-text f-Asap_medium">
                {{$order_process->transaction_code}}
            </h5>
            <h6 class="clr-light-gry text-capitalize mb-0">
                {{ Carbon\Carbon::parse($order_process->created_at)->format('d F Y') }}
            </h6>
        </div>
        <div class="bx-head-mns stts-order">
            ...
            <div class="stts-listjhdd --trsgc">
                ...
                <div class="status-order --process">
                    <h6 class="small-fnt text-capitalize mb-0 stts-order-text">{{$order_process->p_status}}</h6>
                </div>
        ...
        <div class="bx-head-mns ttl-order">
            ...
            <h5 style="color: #FA591D;" class="mb-0 f-Asap_medium desc-main-text">
                Rp. {{ number_format($order_process->grandtotal ,2,",",".")}}
            </h5>
        </div>
    </div>
        ...
        <div class="accordion" id="accordionProduct">
            <div class="card">
                <div class="card-header" id="headingOne">
                    <button class="btn-vegan txt-btn" type="button" data-toggle="collapse" data-target="#listsproducts_{{$order_process->id}}" aria-expanded="true" aria-controls="listsproducts_{{$order_process->id}}">
                        Daftar Belanja (<span id="total_all" class="constantformatnumberCN" data-content="{{count($order_process->details)}}">{{count($order_process->details)}}</span>)
                        <span class="icn-arrow" style="background-image: url('{{ asset('dist/fe/icons/arrow-grey.png') }}')"></span>
                    </button>
                </div>
                <div id="listsproducts_{{$order_process->id}}" class="collapse show" aria-labelledby="headinglistsproducts_{{$order_process->id}}" data-parent="#accordionProduct">
                    <div class="grid_layouts card-body">
                        @if($search == true)
                            @if($order_process->transaction_status == 1 && $order_process->payment_status == 1)
                            <div class="grid_layouts gr-cart-prdct">
                            @else
                            <div class="grid_layouts gr-cart-prdct list-products-all-{{$order_process->id}}" data-qty="{{$order_process->qty}}" data-product="{{$order_process->product_id}}">
                            @endif
                                <div class="cover-img img-lazy small-lazy" data-src="{{ env('APP_DOWNLOAD') . $order_process->media }}"></div>
                                <div class="desc">
                                    <h6 class="prd-title truncate-texts mb-1">
                                        {{ $order_process->name }}
                                    </h6>
                                    <h6 class="prd-price clr-light-gry truncate-texts mb-0">
                                        Rp.{{ number_format($order_process->price ,2,",",".")}} &nbsp;&nbsp; x{{ $order_process->qty}}
                                    </h6>
                                </div>
                            </div>
                        @else
                            @foreach($order_process->details as $item)
                            <div class="grid_layouts gr-cart-prdct list-products-process-{{$order_process->id}}" data-qty="{{$item->qty}}" data-product="{{$item->product_id}}">
    ...
    <div class="grid_layouts footer-pt-list-order">
        <div class="rincian-view-btns">
            <a href="{{route('akun_pesanan.detail', $order_process->transaction_code)}}" class="view-rincian btn-vegan txt-btn text-capitalize">
...
@push('scripts')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            /*Rebuy Produk*/
            /********************************/
            $('.rebuymainorder_process').on('click',function (e) {
                e.preventDefault();
                var id = $(this).data('id');
                var id_product   = [];
                var qty_buy_again   = [];
                var hasilchck = $(this).data('price');
                $.each($('.list-products-process-'+id+''),function(index){
