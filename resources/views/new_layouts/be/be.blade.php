...
<html>
    ...
    <body class="app">
        ...
        <form id="formLogout" action="{{ route('be.logout') }}" method="POST" style="display: none;">
            @csrf
        </form>
        <!-- ============================================================== -->
        <!-- All Jquery -->
        ...
        <script type="text/javascript">
            ...
            function submitData(id_form, redirect = null, reload_table = null) {
                $(document).on('submit','#'+id_form, function (e) {
                    ...
                    if (typeof table_cover !== 'undefined') {
                        // the variable is defined
                        table_cover.rows().every(function (e) {
                            let temp = [];
                            let d = this.data();
                            let i = d.no-1;
                            data.append("images[foto]["+i+"]", d.rawfoto);
                            data.append("images[is_edit]["+i+"]", d.is_edit);
                            data.append("images[cover_id]["+i+"]", d.cover_id);
                            data.append("images[tipe]["+i+"]", d.tipe);
                        });
                    }

                    if (typeof blog_post_ids != 'undefined') {
                        let json_arr = JSON.stringify(blog_post_ids);
                        data.append("blog_post_ids", json_arr);
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            function (isConfirm) {
                                if (isConfirm && response.type != 'error') {
                                    if (redirect || response.redirect) {
                                        window.location = redirect ? redirect : response.redirect;
                                    } else {
                                        if (reload_table) {
                                            tableReload();
                                        } else {
